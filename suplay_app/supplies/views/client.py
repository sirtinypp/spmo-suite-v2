...
def check_monthly_allocation(user, product_id, quantity_to_add, current_cart_qty=0):
    ...
    try:
        # 1. Get Allocation
        alloc = AnnualProcurementPlan.objects.get(
            department=user_dept, product_id=product_id, year=current_year
        )
        ...
        consumed = (
            OrderItem.objects.filter(
                order__in=monthly_orders, product_id=product_id
        ...
        if total_attempted > limit:
            ...
            msg = (
                f"Monthly Limit Reached ({month_str.title()})! "
                f"Allocation: {limit}. "
        ...
        return (
            False,
            "Restricted: Your department has no allocation record for this item this year.",
            0,
        )
...
def home(request):
    ...
    if request.user.is_superuser:
        ...
        allocated_product_ids = AnnualProcurementPlan.objects.filter(
            department=user_dept, year=current_year
        ...
        if not allocated_product_ids:
            ...
            allocated_product_ids = AnnualProcurementPlan.objects.filter(
                department=user_dept, year=previous_year
            ).values_list("product_id", flat=True)
        ...
        if not allocated_product_ids:
            ...
            allocated_product_ids = Product.objects.values_list("id", flat=True)
            info_message = (
                f"No allocations found for {user_dept.name}. Showing all products."
            )
        else:
            # Base queryset - ONLY allocated products
            products = (
                Product.objects.filter(id__in=list(allocated_product_ids))
                .select_related("category", "supplier")
                .order_by("name")
            )
    else:
        ...
        for p in products:
            try:
                plan = AnnualProcurementPlan.objects.get(
                    department=user_dept, product=p, year=current_year
                )
            ...
            consumed = (
                OrderItem.objects.filter(order__in=monthly_orders, product=p).aggregate(
                    total=Sum("quantity")
                )["total"]
                or 0
            )
    ...
    if user_dept or request.user.is_superuser:
        categories = (
            Category.objects.annotate(
                product_count=Count(
                    "product", filter=Q(product__id__in=allocated_product_ids)
                )
            )
        ...
        categories = Category.objects.annotate(product_count=Count("product")).order_by(
            "name"
        )  # Fallback if no dept (should be empty products anyway)
    ...
    newly_added = Product.objects.filter(
        created_at__gte=thirty_days_ago, id__in=product_ids
...
def search(request):
    ...
    if query:
        ...
        if user_dept:
            ...
            for p in products:
                ...
                consumed = (
                    OrderItem.objects.filter(
                        order__in=monthly_orders, product=p
                    ).aggregate(total=Sum("quantity"))["total"]
                    or 0
                )
    ...
    return render(
        request,
        "supplies/home.html",
        {"products": products, "categories": categories, "search_query": query},
    )
...
def view_cart(request):
    ...
    return render(
        request, "supplies/cart.html", {"cart_items": cart_items, "total": total}
    )
    ...
    if request.method == "POST":
        ...
        for pid, qty in cart.items():
            try:
                ...
                is_allowed, _, _ = check_monthly_allocation(
                    request.user, pid, qty, current_cart_qty=0
                )
                if not is_allowed:
                    continue
        ...
        for pid, prod in valid_items.items():
            ...
            OrderItem.objects.create(
                order=order, product=prod, quantity=quantity, price=prod.price
            )
...
def profile(request):
    ...
    if last_order:
        ...
        if last_item:
            ...
            suggested_products = (
                Product.objects.filter(category=target_category)
                .exclude(id=last_item.product.id)
                .order_by("?")[:4]
            )
    ...
    if not suggested_products or len(suggested_products) < 4:
        ...
        fallback_items = Product.objects.exclude(id__in=ids_to_exclude).order_by("?")[
            :additional_needed
        ]
        # Combine them
        import itertools
    ...
    return render(
        request,
        "supplies/profile.html",
        {"orders": my_orders, "suggested_products": suggested_products},
    )
