    ...
    def before_import(self, dataset, **kwargs):
        ...
        if "Office" in headers and "Month" in headers:
            ...
            for row in dataset:
                ...
                office_name = DEPARTMENT_MAPPING.get(
                    raw_office, raw_office
                )  # Map or keep original

                # B. Get Item Code
                item_code = row_dict.get(
                    "Item Code PSDB", row_dict.get("Item Code", "")
                ).strip()
                ...
                try:
                    row_year = int(
                        str(
                            row_dict.get(
                                "Year", row_dict.get("year", timezone.now().year)
                            )
                        ).strip()
                    )
                except Exception:
                ...
                try:
                    qty = int(
                        float(
                            str(
                                row_dict.get("QTY", row_dict.get("Quantity", 0))
                            ).replace(",", "")
                        )
                    )
                except ValueError:
            ...
            import_codes = set(
                d["Item Code"] for d in aggregated_data.values() if d["Item Code"]
            )

            # Find which ones exist in DB
            existing_codes = set(
                Product.objects.filter(item_code__in=import_codes).values_list(
                    "item_code", flat=True
                )
            )
...
class APPAdmin(ImportExportModelAdmin):
    ...
    def export_as_flat_csv(self, request, queryset):
        ...
        response["Content-Disposition"] = (
            'attachment; filename="APP_Allocation_Flat.csv"'
        )
        ...
        for obj in queryset:
            ...
            date_req = (
                obj.date_requested.strftime("%Y-%m-%d") if obj.date_requested else ""
            )
