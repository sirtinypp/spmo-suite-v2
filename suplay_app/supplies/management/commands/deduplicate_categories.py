    ...
    def handle(self, *args, **kwargs):
        ...
        for norm_name, cats in normalized_map.items():
            if len(cats) > 1:
                ...
                def score_cat(c):
                    s = 0
                    if c.name == c.name.title(): s = 10
                    elif c.name == c.name.capitalize(): s = 5
                    return (s, -c.id) # Sort descending by score, then ascending by ID (via negative ID text? No, Python sort tuples properly)
                ...
                cats.sort(key=lambda c: (10 if c.name == c.name.title() else (5 if c.name == c.name.capitalize() else 0), -c.id), reverse=True)
                ...
                for spare in spares:
                    ...
                    if count > 0:
                        ...
                        self.stdout.write(f"  - Moved {count} products from '{spare.name}' (ID {spare.id}) to keeper.")
                    else:
                        self.stdout.write(f"  - '{spare.name}' (ID {spare.id}) is empty.")
