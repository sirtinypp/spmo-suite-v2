...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        product, _ = Product.objects.get_or_create(
            name="Test Restricted Item", defaults={"price": 500, "category": category}
        )
        ...
        if response.status_code == 400:
            ...
            self.stdout.write(
                self.style.SUCCESS(f"SUCCESS: Blocked Payload: {content['message']}")
            )
        elif response.status_code == 200:
            self.stdout.write(
                self.style.ERROR(
                    f"FAILURE: Allowed Payload: {response.content.decode()}"
                )
            )
        else:
            self.stdout.write(
                self.style.WARNING(f"Unexpected Status: {response.status_code}")
            )
