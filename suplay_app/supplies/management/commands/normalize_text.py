...
from supplies.models import Category, Supplier, Department, Product, AnnualProcurementPlan, Order, OrderItem, StockBatch, UserProfile
...
class Command(BaseCommand):
    ...
    def normalize_model(self, model_class, field_name):
        ...
        for obj in all_objs:
            ...
            collision = model_class.objects.filter(**{field_name: target_val}).exclude(id=obj.id).first()
        ...
        self.stdout.write(f"  {model_name}: {renamed_count} renamed, {merged_count} merged.")
    ...
    def merge_objects(self, model_class, source, target):
        ...
        with transaction.atomic():
            if model_class == Category:
                ...
                StockBatch.objects.filter(supplier_name=source.name).update(supplier_name=target.name) # Note: StockBatch uses CharField name not FK sometimes? Checking... StockBatch doesn't have supplier FK?
                # Checked models.py: StockBatch has no supplier FK, just 'supplier_name'. We update it textually.
            
            elif model_class == Department:
                Product.objects.filter(category__name="Uncategorized").update(category=None) # irrelevant
                # Update references
                UserProfile.objects.filter(department=source).update(department=target)
                AnnualProcurementPlan.objects.filter(department=source).update(department=target) # Warning: UniqueTogether constraint might fail here.
