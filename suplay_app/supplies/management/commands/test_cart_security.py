...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        product, _ = Product.objects.get_or_create(name='Sec Item', defaults={'price': 100, 'category': cat, 'stock': 100})
        ...
        if qty == 100:
            self.stdout.write(self.style.ERROR(f"❌ VULNERABILITY CONFIRMED: Cart updated to 100 despite limit 10."))
        else:
            self.stdout.write(self.style.SUCCESS(f"✅ SECURE: Cart quantity is {qty} (Expected < 10)"))
        ...
        if latest_order and latest_order.total_amount == 100 * 100:
             self.stdout.write(self.style.ERROR(f"❌ VULNERABILITY CONFIRMED: Order #{latest_order.id} created for 100 items."))
        else:
             self.stdout.write(self.style.SUCCESS(f"✅ SECURE: Checkout blocked or order not created."))
