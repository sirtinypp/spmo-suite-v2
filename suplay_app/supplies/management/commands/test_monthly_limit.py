...
from supplies.models import Product, Department, AnnualProcurementPlan, UserProfile, Category, Order, OrderItem
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        product, _ = Product.objects.get_or_create(name='Test Month Item', defaults={'price': 100, 'category': category, 'stock': 100})
        ...
        plan, created = AnnualProcurementPlan.objects.get_or_create(
            department=dept, product=product, year=now.year,
            defaults={} 
        )
        ...
        if response.status_code == 400:
             ...
             self.stdout.write(self.style.SUCCESS(f"SUCCESS: Blocked. Message: {content['message']}"))
        else:
             self.stdout.write(self.style.ERROR(f"FAILURE: Status {response.status_code}, Content: {response.content.decode()}"))
        ...
        if response.status_code == 200:
             content = json.loads(response.content.decode('utf-8'))
             self.stdout.write(self.style.SUCCESS(f"SUCCESS: Added. Cart Count: {content['cart_count']}"))
        else:
             self.stdout.write(self.style.ERROR(f"FAILURE: Status {response.status_code}, Content: {response.content.decode()}"))
        ...
        if response.status_code == 200:
            ...
            for p in products:
                if p.id == product.id:
                    ...
                    # Limit = 10.
                    # Remaining = 0.
                    self.stdout.write(f"Product Found: {p.name}. Personal Stock: {getattr(p, 'personal_stock', 'MISSING')}")
                    # Cart is empty in this new Client session, so remaining is 10 - 5 = 5.
                    if getattr(p, 'personal_stock', -1) == 5:
                        self.stdout.write(self.style.SUCCESS(f"SUCCESS: Personal Stock is 5 (Correct: 10 Alloc - 5 Consumed - 0 Cart)"))
                    else:
                         self.stdout.write(self.style.ERROR(f"FAILURE: Personal Stock is {getattr(p, 'personal_stock', 'MISSING')}, expected 5"))
                    break
            if not found:
                 self.stdout.write(self.style.ERROR("FAILURE: Test Product not found in home view context."))
        else:
             self.stdout.write(self.style.ERROR(f"FAILURE: Home view returned {response.status_code}"))
