...
from supplies.models import Product, Department, AnnualProcurementPlan, UserProfile, Category, Supplier
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        p1, _ = Product.objects.get_or_create(name='Allocated Pen', defaults={
            'price': 10, 'category': cat_a, 'supplier': sup_x, 'stock': 50, 'item_code': 'CODE123'
        })
        ...
        p2, _ = Product.objects.get_or_create(name='Allocated Paper', defaults={
            'price': 20, 'category': cat_b, 'stock': 0
        })
        p2.category = cat_b
        p2.stock = 0
        p2.save()
        ...
        p3, _ = Product.objects.get_or_create(name='Hidden Marker', defaults={
            'price': 5, 'category': cat_a, 'stock': 100
        })

        # Setup Allocations (Only for P1 and P2)
        now = timezone.now()
        defaults = {m: 10 for m in ['jan','feb','mar','apr','may','jun','jul','aug','sep','oct','nov','dec']}
        
        AnnualProcurementPlan.objects.get_or_create(department=dept, product=p1, year=now.year, defaults=defaults)
        AnnualProcurementPlan.objects.get_or_create(department=dept, product=p2, year=now.year, defaults=defaults)
        ...
        if p1 in prods and p2 not in prods and p3 not in prods:
            self.stdout.write(self.style.SUCCESS("✅ Pass: Shows P1 (Alloc+Stock), Hides P2 (No Stock), Hides P3 (Unalloc)"))
        else:
            self.stdout.write(self.style.ERROR(f"❌ Fail: Got {[p.name for p in prods]}"))
        ...
        if p1 in prods and p2 not in prods:
             self.stdout.write(self.style.SUCCESS("✅ Pass: Shows P1 (Cat A), Hides P2 (Cat B)"))
        else:
             self.stdout.write(self.style.ERROR(f"❌ Fail: Got {[p.name for p in prods]}"))
        ...
        if p1 in prods and p2 in prods:
             self.stdout.write(self.style.SUCCESS("✅ Pass: Search finds both Allocated items (In/Out Stock)"))
        else:
             self.stdout.write(self.style.ERROR(f"❌ Fail: Got {[p.name for p in prods]}"))
        ...
        prods = list(resp.context['products'])
        if not prods:
             self.stdout.write(self.style.SUCCESS("✅ Pass: Search blocked Unallocated item"))
        else:
             self.stdout.write(self.style.ERROR(f"❌ Fail: Found restricted item {[p.name for p in prods]}"))
