...
class Command(BaseCommand):
    help = (
        "Advanced Deduplication: Merges complex variations and generates descriptions."
    )
    ...
    def normalize_ampersands(self):
        ...
        for spare in cats_with_amp:
            ...
            target = (
                Category.objects.filter(name__iexact=new_name)
                .exclude(id=spare.id)
                .first()
            )
    ...
    def merge_category(self, spare, target):
        with transaction.atomic():
            ...
            self.stdout.write(
                f"  - Moved {count} products from '{spare.name}' (ID {spare.id})"
            )
            spare.delete()
