...
class Command(BaseCommand):
    help = (
        "Deduplicates Products based on item_code, keeping the one with most stock/ID."
    )

    def handle(self, *args, **kwargs):
        self.stdout.write(
            "--- Aggressive Deduplication (Case-Insensitive + Stripped) ---"
        )
        ...
        for code_key, products in code_map.items():
            if len(products) > 1:
                ...
                for spare in spares:
                    self.stdout.write(
                        f"  - Merging Spare ID {spare.id} ({spare.item_code})..."
                    )
                    ...
                    for app in apps:
                        existing = AnnualProcurementPlan.objects.filter(
                            product=keeper, department=app.department, year=app.year
                        ).first()
                        if existing:
        ...
        if duplicates_found == 0:
            self.stdout.write(
                self.style.SUCCESS("No duplicates found with case-insensitive check!")
            )
        else:
            self.stdout.write(
                self.style.SUCCESS(
