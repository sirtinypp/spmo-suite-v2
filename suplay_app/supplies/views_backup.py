...
def home(request):
    ...
    if user_department:
        ...
        allocated_product_ids = AnnualProcurementPlan.objects.filter(
            department=user_department, year=2025  # Default Year
        ...
        allocations = AnnualProcurementPlan.objects.filter(
            department=user_department, year=2025
    ...
    if stock_status == "in":
        if isinstance(products, list):
            products = [
                p for p in products if (getattr(p, "personal_stock", p.stock) > 0)
            ]
        else:
    ...
    restocked_batches = StockBatch.objects.select_related("product").order_by(
        "-date_received"
    )[:5]
...
def transaction_list(request):
    ...
    departments = (
        Order.objects.values_list("department", flat=True)
        .distinct()
        .order_by("department")
    )
...
def delivery_dashboard(request):
    ...
    context = {
        ...
        "count_today_completed": Order.objects.filter(
            status="delivered", completed_at__date=timezone.now().date()
        ).count(),
        "selected_dept": dept_filter,
        "selected_date_from": date_from,
        "selected_date_to": date_to,
...
def edit_product(request, pk):
    ...
    return render(
        request, "supplies/product_form.html", {"form": form, "title": "Edit Product"}
    )
