...
def run():
    ...
    with transaction.atomic():
        ...
        app_depts = (
            AnnualProcurementPlan.objects.exclude(department__isnull=True)
            .exclude(department="")
        ...
        for dept_name in app_depts:
            ...
            count = AnnualProcurementPlan.objects.filter(department=dept_name).update(
                department_new=dept_obj
            )
        ...
        print("\nMigrating Orders...")
        order_depts = (
            Order.objects.exclude(department__isnull=True)
            .exclude(department="")
            .values_list("department", flat=True)
            .distinct()
        )
        ...
        for dept_name in order_depts:
            dept_obj, created = Department.objects.get_or_create(name=dept_name.strip())
            # Update Orders
            count = Order.objects.filter(department=dept_name).update(
                department_new=dept_obj
            )
        ...
        for dept_name in profile_depts:
            dept_obj, created = Department.objects.get_or_create(name=dept_name.strip())
            # Update Profiles
            count = UserProfile.objects.filter(department=dept_name).update(
                department_new=dept_obj
            )
