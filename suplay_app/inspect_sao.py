...
try:
    ...
    if hasattr(user, 'profile') and user.profile.department:
        ...
        years = AnnualProcurementPlan.objects.filter(department=dept).values('year').annotate(count=Count('id'))
        if years:
            for y in years:
        ...
        relevant_allocs = AnnualProcurementPlan.objects.filter(department=dept, year=sys_year)
        ...
        if count_relevant == 0 and years:
             ...
             print(f"User has data for Year {likely_year}, but Code looks for {sys_year}.")
        ...
        alloc_ids = AnnualProcurementPlan.objects.filter(department=dept, year=sys_year).values_list('product_id', flat=True)
