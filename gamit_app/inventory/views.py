...
def dashboard(request):
    ...
    if request.user.is_superuser and not demo_role:
        ...
        # Unit Persona View: Filter by a fixed demo department (PGC - ID 127)
        from .models import Department
        try:
            demo_dept = Department.objects.get(id=128)
            assets = assets.filter(department=demo_dept)
            user_department = demo_dept
    ...
    context = {
        ...
        'highest_name': highest_name, 'is_unit_view': demo_role.startswith('UNIT_') if demo_role else False,
...
def get_user_tab_permissions(user, role_override=None):
    ...
    demo_role = role_override if role_override else getattr(user, 'active_demo_role', None)
    if demo_role:
...
def asset_detail(request, pk):
    ...
    if request.user.is_superuser and not demo_role:
        ...
        # Unit Persona: Strict Department Isolation (Fixed to UPRI ID 128)
        from .models import Department
        try:
            demo_dept = Department.objects.get(id=128)
            if asset.department != demo_dept:
    ...
    # Role permissions
    tab_perms = get_user_tab_permissions(request.user, role_override=request.session.get('active_demo_role'))
    ...
    return render(request, 'inventory/asset_detail.html', {
        'asset': asset, 'is_unit_view': request.session.get('active_demo_role', '').startswith('UNIT_'),
...
def add_asset_transaction(request):
    ...
    if request.method == 'POST':
        form = AddAssetForm(request.POST, files=request.FILES)
        if form.is_valid():
...
def create_inspection_request(request):
    if request.method == 'POST':
        form = InspectionRequestForm(request.user, demo_role=request.session.get("active_demo_role"), data=request.POST, files=request.FILES)
        if form.is_valid():
        ...
        form = InspectionRequestForm(request.user, demo_role=request.session.get("active_demo_role"))
...
def create_batch_request(request):
    ...
    if request.method == 'POST':
        form = AssetBatchForm(request.POST, files=request.FILES)
        formset = BatchItemFormSet(request.POST, files=request.FILES)
...
def create_transfer_request(request):
    if request.method == 'POST':
        form = AssetTransferRequestForm(request.user, demo_role=request.session.get("active_demo_role"), data=request.POST, files=request.FILES)
        if form.is_valid():
        ...
        form = AssetTransferRequestForm(request.user, demo_role=request.session.get("active_demo_role"))
...
def add_service_log(request, pk):
    asset = get_object_or_404(Asset, pk=pk)
    if request.method == 'POST':
        form = ServiceLogForm(request.POST, files=request.FILES)
        if form.is_valid():
...
def create_return_request(request):
    if request.method == 'POST':
        form = AssetReturnRequestForm(request.user, demo_role=request.session.get("active_demo_role"), data=request.POST, files=request.FILES)
        if form.is_valid():
        ...
        form = AssetReturnRequestForm(request.user, demo_role=request.session.get("active_demo_role"))
...
def create_loss_report(request):
    if request.method == 'POST':
        form = AssetLossReportForm(request.user, demo_role=request.session.get("active_demo_role"), data=request.POST, files=request.FILES)
        if form.is_valid():
            req = form.save(commit=False)
            req.requestor = request.user
            req.save()
        ...
        form = AssetLossReportForm(request.user, demo_role=request.session.get("active_demo_role"))
...
def create_clearance_request(request):
    if request.method == 'POST':
        form = PropertyClearanceRequestForm(request.POST, files=request.FILES)
        if form.is_valid():
            req = form.save(commit=False)
            req.requestor = request.user
            req.save()
...
def rpcppe_report(request):
    ...
    if demo_role and demo_role.startswith('UNIT_'):
        # Unit Persona: Strict Department Isolation (Fixed to PGC ID 127)
        from .models import Department
        try:
            demo_dept = Department.objects.get(id=128)
            assets_qs = assets_qs.filter(department=demo_dept)
        except Department.DoesNotExist:
...
def bulk_media_upload(request):
    ...
    # --- AJAX CHUNK HANDLER (POST) ---
    if request.method == 'POST' and request.headers.get('X-Requested-With') == 'XMLHttpRequest':
        images = request.FILES.getlist('images')
        upload_type = request.POST.get('upload_type', 'condition')
        chunk_results = []
        ...
        for image in images:
            ...
            if not match:
                chunk_results.append({'file': filename, 'status': 'Error', 'msg': 'No numeric PAR found in name.'})
                continue
            ...
            if asset:
                try:
                    ...
                    asset.save()
                    chunk_results.append({'file': filename, 'status': 'Success', 'asset': asset.property_number})
                except Exception as e:
                    chunk_results.append({'file': filename, 'status': 'Error', 'msg': str(e)})
            else:
                chunk_results.append({'file': filename, 'status': 'Not Found', 'msg': f'No Asset matches digits: {par_digits}'})

        return JsonResponse({'results': chunk_results})
    ...
    metrics = {
        ...
        'complete_list': complete_assets_qs[:100],
