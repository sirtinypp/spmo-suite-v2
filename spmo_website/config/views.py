...
def home(request):
    ...
    # 1. Fetch Latest News
    try:
        news_list = NewsPost.objects.filter(is_published=True).order_by("-date_posted")[
            :3
        ]
    except:
        news_list = []
    ...
    try:
        activity_list = Activity.objects.filter(
            is_public=True, start_date__gte=now
...
def admin_portal(request):
    ...
    active_assets = get_external_count(
        "db_gamit", "SELECT COUNT(*) FROM inventory_asset WHERE status != 'DISPOSED'"
    )
    pending_inspections = get_external_count(
        "db_gamit",
        "SELECT COUNT(*) FROM inventory_inspectionrequest WHERE status = 'Pending Inspection'",
    )

    # 2. SUPLAY Metrics (Supplies)
    # Using raw SQL to check stock <= reorder_point
    critical_stock = get_external_count(
        "db_store", "SELECT COUNT(*) FROM supplies_product WHERE stock <= reorder_point"
    )
    pending_orders = get_external_count(
        "db_store", "SELECT COUNT(*) FROM supplies_order WHERE status = 'pending'"
    )

    # 3. LIPAD Metrics (Travel)
    active_travels = get_external_count(
        "db_gfa",
        "SELECT COUNT(*) FROM travel_bookingrequest WHERE status IN ('APPROVED', 'BOOKED')",
    )
    pending_bookings = get_external_count(
        "db_gfa", "SELECT COUNT(*) FROM travel_bookingrequest WHERE status = 'PENDING'"
    )
...
class NewsArchiveView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["current_year"] = (
            int(year_param) if year_param and year_param.isdigit() else None
        )
