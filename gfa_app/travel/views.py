...
def admin_dashboard(request):
    ...
    if user.is_superuser:
        ...
        transactions = BookingRequest.objects.filter(unit_office=user_office).order_by('-created_at')
        dashboard_title = f"{user.userprofile.get_office_display()} Dashboard"
        template_name = 'travel/dashboard.html'
    else:
        transactions = BookingRequest.objects.filter(created_by=user).order_by('-created_at')
    ...
    if status_filter: transactions = transactions.filter(status=status_filter)
    date_filter = request.GET.get('date')
    if date_filter: transactions = transactions.filter(departure_date=date_filter)
    ...
    if template_name == 'travel/dashboard.html':
        ...
        # Charts
        dest_data = transactions.values('destination_details').annotate(count=Count('id'))
    ...
    context = {
        'transactions': transactions, 'total_reqs': total_reqs, 'pending_reqs': pending_reqs,
        ...
        'search_query': search_query, 'current_status': status_filter, 'current_date': date_filter
    }
