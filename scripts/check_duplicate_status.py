...
def check():
    ...
    matches = list(Product.objects.filter(item_code__icontains=target).values_list('item_code', flat=True))
    ...
    dupes = Product.objects.values('item_code').annotate(count=Count('id')).filter(count__gt=1)
